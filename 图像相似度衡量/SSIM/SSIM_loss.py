...
def tf_ssim(img1, img2, cs_map=False, mean_metric=True, size=11, sigma=1.5):
    ...
    if cs_map:
        ...
        # value = ((2*mu1_mu2 + C1)*(2*sigma12 + C2))/((mu1_sq + mu2_sq + C1)*
        #             (sigma1_sq + sigma2_sq + C2))
        value = (sigma12+C2)/(tf.sqrt(sigma1_sq*sigma2_sq)+C2)
...
img = np.asarray(Image.open('imgs/#1_50_TVSmooth.png'),dtype=float)
# img = img[:,:,0]
rows, cols = img.shape[0],img.shape[1]

# img_noise = np.asarray(Image.open("imgs/#2_50_TVSmooth.png"),dtype=float)
...
ssim_index = tf_ssim(image4d_1, image4d_2,mean_metric=True)
